...
void NodeAttributeGenerator::generateRandomAttributes(const std::string &a_TypeName,
    ...
    if (!attributes.empty()) {
        ...
        for (const auto &attribute : attributes) {
            generateNodeAttributes(attribute, type_range.first, type_range.second, a_TypeName, a_OutputStream);
        }
    }
}

void NodeAttributeGenerator::generateNodeAttributes(const std::unique_ptr<Attribute> &a_Attribute, const int a_StartId,
                                                    const int a_EndId, const std::string &a_TypeName, std::ostream &a_OutputStream) const {
    ...
    for (int node_id = a_StartId; node_id <= a_EndId; ++node_id) {
        ...
        a_OutputStream << a_TypeName << "_" << node_id << ',' << attribute_name << ',' << random_attribute << "\n";
    }
}
